...
-- The difference between left joins and inner joins, is that inner joins provide the intersection of two tables, 
-- which are the two rows they have in common. Left joins will select all of the rows in one of the selected tables 
-- (which can be described as the "primary" table: for example, table "A" if it is a left join or table "B" if it is another left join), 
-- and will "join" the items from the other, secondary table, but only the items that the secondary table has in common with the primary table.
